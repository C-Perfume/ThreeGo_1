...
public class CPlayRoomUI : MonoBehaviour, IMessageReceiver {
	...
	void make_deck_cards()
	{
		...
		for (int i = 0; i < this.total_card_pictures.Count; ++i)
		{
			...
			//this.total_card_pictures[i].transform.rotation = Quaternion.identity;  ㅂ 주석처리
			this.total_card_pictures[i].transform.eulerAngles = new Vector3(90, 0);
			...
			foreach(KeyValuePair<byte, Queue<CCard>> kvp in player_cards)
			{
				...
				for (int card_index = 0; card_index < 5; ++card_index)
				{
					...
					if (player_index == this.player_me_index)
					{
						...
						//ㅂ
						card_picture.transform.eulerAngles = new Vector3(30, 0);
	...
	IEnumerator move_flip_card(byte number, PAE_TYPE pae_type, byte position)
	{
		...
		//ㅂ
		deck_card_picture.transform.eulerAngles = new Vector3(30, 0);
		yield return StartCoroutine(flip_deck_card(deck_card_picture));
		...
		//ㅂ
		deck_card_picture.transform.eulerAngles = new Vector3(90, 0);
		...
		byte player_card_position)
	{
		...
		for (int i = 0; i < targets.Count; ++i)
		{
			...
			//ㅂ
			player_card.transform.eulerAngles = new Vector3(90, 0);
		}
	}
